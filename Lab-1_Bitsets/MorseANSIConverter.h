...
private:
	...
	short morseHash(std::bitset<2> morseCache) { return static_cast<short>(morseCache.to_ulong()); }

	// used for converting 2 bits to a Morse code at a time
	std::bitset<2> morseCache;
	std::bitset<8> ansiCache;
public:
	...
	const std::bitset<8> & checkANSICache() const { return ansiCache; }
